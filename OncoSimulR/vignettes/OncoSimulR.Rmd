...
## Growth factors as chemotherapy target, example 2 {#ex2}

As it has been reported in the literature and proven in the above simulations, resistance acquisition is almost inevitable. A new approach for avoiding this evolutionary adaptation proposes changing the chemotherapy target from the subclones to the growing factors (GF) they produce. These molecules, which are secreted to the medium by **cooperators**, help grow not just their own subclone types, but also the GF **defectors**. With this, the fitness of the whole population increases and the tumour expands.

An attended side effect of this type of treatment is the emergence of cooperators that overproduce GF. This will increase their fitness and reduce the impact of GF sequestering agents. However, it will also increase the cost of its production, which will smooth this benefitial impact.

@Archetti2013 proposes a series of formulas to relate the impact of cooperation in tumour composition and fitness of subclones, which they divide in cooperators and defectors. Although GFs are not always distributed in a homogeneously, we will assume they do in the next simulations for the sake of simplicity.  
...
The system payoff matrix is the following:
  ...
  ```
⎡a b c m⎤      ⎡ 1   0.5  0.5  0.75⎤
A =  |d e f o|  =    | 1  1.25 0.7  0.185|
  |g h i p|       | 1   1.5 0.5   2.5 |
  ⎣j k l q⎦      ⎣0.8   1   0.5  1.5 ⎦
```

The fitness definitions for the four types of cells are:
  
  F(WT) = a*x(WT) + b*x(C) + c*x(D) + m*x(P)
F(C) = d*x(WT) + e*x(C) + f*x(D) + o*x(P)
F(D) = g*x(WT) + h*x(C) + i*x(D) + p*x(P)
F(P) = j*x(WT) + k*c(C) + l*x(D) + q*x(P)

Ordered by decreasing fitness:
  
  In summary:
  
  * The fitness of WT cells is lower than that of tumour cells
* With no therapy, C cells fitness is higher than P cells', so the overproducers will not overtake them.
* D subclones benefit from high concentration of cooperators (C or P).

### Scenario without therapy {#ex2notherapy}

First, a study of the fitness of our subclones based on hypothetical frequencies will be performed. Then, a simulation of the growth of the tumour under no treatment will be carried out. We consider that C cells are the original tumour cells and that mutagenesis can lead to the loss of the GF gene (generating D) or the duplication of this same gene (generating P).
...
game_no <- allFitnessEffects(genotFitness = coop_no,
                          frequencyDependentFitness = TRUE,
                          frequencyType = "rel",
                          spPopSizes = c(10, 10, 0, 0, 10, 1, 0, 0))

## Plot 1
eag <- evalAllGenotypes(game_no)[c(1, 2, 5, 6),]
plot(eag)
...
The resulting plots show D is highly benefited by the second most common subclone, C, which is a cooperator. On the other hand, subclone P has a lower fitness value because of the GF cost, so it is not the selected type for this environment (although a small number of this subclone is mantained).


### Scenario with GF as target for chemotherapy {#ex2gftherapy}

In this final simulation, we will add the drug effect (at time 50) to the fitness of each subclone. Assuming it will cause a reduction of the fitness differently depending on the level of dependency to GF of each subclone, we have introduced some constants to make it more accurate. As cooperators will keep producing GF, they will be less affected by the sequestration of this molecule, so their coefficients will be greater than 1. However, P will be less affected, since its production of GF is greater, so its coefficient will be greater than the C one (1.5 > 1.1). On the other hand, as D is a defector, it will be more affected by the treatment, so its coefficient is set to 0.9, smaller than 1.

```{r}
## Chemo - Impairing GF

# Effect of drug on GF availability
drug_eff <- 0.25 # This term is multiplied by the fitness, reduces the GF available
...
# The drug effect is 1.5 the original because of the overproduction of GF and the full disponibility of this molecule inside the producing subclone.
...
As expected by @Archetti2018, the tumour will be biased towards P mutant. Even if its fitness is lower than C's because of its overproduction of GF, it will assure its survival and proliferation when chemotherapy is applied. As a result of this, D does not completely disappear (as it has happened to C), but it has drastically reduced its number. 
